...
import { Check, MousePointer, Power, Search, Shield } from "@deemlol/next-icons";
    ...
    <main className="flex-1">
      ...
      <section className="relative py-40 text-center">
        ...
        <Wrapper className="relative z-10 !max-w-3xl">
          <h2
            className="mb-6 text-4xl font-black tracking-tight sm:text-5xl md:text-6xl text-gray-900 dark:text-white">
          ...
          {benefitsData.map((item) => (
            <Card key={item.name} className="w-full max-w-md !rounded-2xl">
              <div className="flex flex-col">
                <div
                  className="mb-3 flex h-12 w-12 items-center justify-center rounded-full bg-primary-50 dark:bg-primary-500/15 text-primary-500 dark:text-primary-400">
                  {item.icon}
                </div>
        ...
        <div className="flex flex-col gap-12">
          {howItWorksData.map((item, index) => (
            <div key={item.name} className="flex gap-5">
              <div
                className="shrink-0 flex h-12 w-12 sm:h-16 sm:w-16 items-center justify-center rounded-full bg-primary-500 text-white text-xl font-bold">
                {index + 1}
              </div>
              ...
              <Card className="h-full">
                <div className="flex flex-col gap-3">
                  <div className="flex items-center gap-3">
                    <div
                      className="w-12 h-12 rounded-full bg-primary-500 flex items-center justify-center text-white font-bold text-lg shrink-0">
                      {name[0]}
                    </div>
                    <div className="flex flex-col">
